...
public class RelationTypeTreeItemController(IRelationService relationService) : RelationsManagerApiControllerBase(relationService)
{
    ...
    public async Task<ActionResult<PagedViewModel<RelationTypeTreeItemResponseModel>>> GetChildren(int skip = 0, int take = 100)
    {
        var relationTypes = RelationService.GetAllRelationTypes().ToList();
